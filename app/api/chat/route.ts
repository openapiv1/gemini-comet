...
const GEMINI_API_KEY = "AIzaSyCNEuCVk-wno4QPWHf6aRSePotWqI18OVc";
...
- Rozdzielczość ekranu: ${resolution.x}x${resolution.y} pikseli
- Współrzędne zaczynają się od (0,0) w lewym górnym rogu
- Maksymalne współrzędne to (${resolution.x-1}, ${resolution.y-1})
...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      // Real-time event sender - no buffering, immediate flush
      const sendEvent = (data: any) => {
        const encoded = encoder.encode(`data: ${JSON.stringify(data)}\n\n`);
        controller.enqueue(encoded);
        // Force flush - ensure data is sent immediately without buffering
      };
        ...
        while (true) {
          ...
          for await (const chunk of result.stream) {
            ...
            for (const part of content.parts) {
              ...
              if (part.functionCall) {
                ...
                // Send arguments as complete JSON - no artificial chunking
                ...
                const executionPromise = (async () => {
                  try {
                    ...
                    if (fc.name === "computer_use") {
                      ...
                      // Dla screenshotów, dodaj obraz do response dla AI
                      if (args.action === "screenshot" && resultData.type === "image") {
                        functionResponses.push({
                          name: fc.name,
                          response: { 
                            result: resultText,
                            image: resultData.data
                          }
                        });
                      } else {
                        functionResponses.push({
          ...
          if (functionCalls.length > 0) {
            ...
            for (const fr of functionResponses) {
              // Jeśli to screenshot z obrazem, dodaj obraz do response dla AI
              if (fr.name === "computer_use" && fr.response.image) {
                responseParts.push({
                  functionResponse: {
                    name: fr.name,
                    response: { result: fr.response.result }
                  }
                });
                // Dodaj obraz jako osobną część
                responseParts.push({
                  inlineData: {
                    mimeType: "image/png",
