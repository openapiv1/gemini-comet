      ...
      <>
        {message.parts.map((part, index) => {
          if (part.type === "tool-invocation") {
            return <div key={`${message.id}-${index}`} className="group/message w-full" data-role={message.role}>{renderToolInvocation(part, props)}</div>;
          } else if (part.type === "text") {
            return (
              <div key={`${message.id}-${index}`} className="group/message w-full" data-role={message.role}>
                <div
                  className={cn(
  ...
  return (
    <div className="group/message w-full" data-role={message.role}>
      ...
      >
        ...
        >
          {/* Dla wiadomości asystenta ze statusem streaming, pokazuj loader obok treści */}
          {props.status === "streaming" && props.isLatestMessage && message.role === "assistant" && (
            <div className="flex items-center gap-2 mb-2">
              {streamingSpinner}
              <span className="text-xs text-zinc-500">Generowanie...</span>
            </div>
          )}
